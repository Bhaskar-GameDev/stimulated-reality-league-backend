...
try {
  ...
  console.warn("Firebase not initialized. Will only update local file.");
}
...
async function processLineups() {
  for (const line of lines) {
    const trimmed = line.trim();
    if (!trimmed) continue;

    // Check if it's a team line (starts with emoji or is in the known list)
      ...
      if (currentTeamPlayers.length === 11) {
        ...
        for (let i = 0; i < currentTeamPlayers.length; i++) {
          ...
          if (player) {
            ...
            // Generate or find from existing saved lineup if available
            ...
            if (!teamKey) {
                // Silent for missing teams as it's expected
            } else {
                console.warn(`Could not find player: ${playerName} in team: ${teamKey}. Using: ${fallbackId}`);
            }
          }
        }
        ...
        if (db) {
            try {
                await db.ref(`lineups/matches/${displayName}`).set(selectedIds);
                console.log(`Pushed ${displayName} lineup to Firebase.`);
            } catch (err) {
                console.error(`Failed to push ${displayName} to Firebase:`, err.message);
            }
        }
      }
    }
