...
async function simulateInnings(matchId, inningNumber, batting, bowling, options = {}) {
  const { oversLimit = 20, delayMs = 0, rng, chaseTarget = null, battingTeamName = "Team A", bowlingTeamName = "Team B", venue = "the stadium", abortSignal } = options;
  ...
  while (currentOver < oversLimit && wickets < 10 && (chaseTarget === null || runs < chaseTarget)) {
    ...
    legalBallsInOver = 0;
    while (legalBallsInOver < 6 && wickets < 10 && (chaseTarget === null || runs < chaseTarget)) {
      ...
      updates[`matches/${matchId}/commentary/${commId}`] = commentaryEngine.generate(matchId, {
          ...
          state: { runs, wickets, over: currentOver, ball: legalBallsInOver, target: chaseTarget }
      });
